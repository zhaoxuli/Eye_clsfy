...
def get_symbol(data, labels):
	...
	conv3 = Conv (data = conv2 ,num_filter = 16 ,kernel=(5,5),stride = (1,1) , pad = (2,2) ,name = 'conv3',pooling_type = 'max')
...
solver = mx.mod.Module(symbol=net, data_names=['data'], label_names=['softmax_label'], context=mx.gpu(2))
solver.fit(
    train_data=train_data_iter,
           ...
           {'learning_rate' : 0.001,
            'momentum' : 0.9,
		   'lr_scheduler' : mx.lr_scheduler.FactorScheduler(step=400000, factor=0.5),
		   'wd' : 0.004
           },
