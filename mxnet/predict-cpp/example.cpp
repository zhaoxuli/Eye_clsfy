...
void GetMeanFile(const std::string image_file, mx_float* image_data,
    ...
    mx_float* ptr_image_g = image_data + 1;
    mx_float* ptr_image_b = image_data + 2;
...
int main(int argc, char* argv[]) {
    ...
    BufferFile param_data("model/Inception/Inception_BN-0039.params_nhwc");
    ...
    int ret = 0;
    //-- Create Predictor
    ret = MXPredCreate((const char*)json_data.GetBuffer(),
    ...
    //-- Set Input Image
    ret = MXPredSetInput(out, "data", image_data.data(), image_size);
    if (ret < 0) {
      std::cerr << "Failed to set input!" << std::endl;
      assert(false);
    }
 
    ret = MXPredForward(out);
    if (ret < 0) {
    ...
    //-- Get Output Result
    ret = MXPredGetOutputShape(out, output_index, &shape, &shape_len);
    if (ret < 0) {
      std::cerr << "Failed to get output shape!" << std::endl;
      assert(false);
    }
    ...
    ret = MXPredGetOutput(out, output_index, &(data[0]), size);
    if (ret < 0) {
    ...
    ret = MXPredFree(out);
    if (ret < 0) {
      std::cerr << "Failed to release predictor!" << std::endl;
      assert(false);
    }
